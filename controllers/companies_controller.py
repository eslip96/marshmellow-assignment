...
def update_company(req, company_id):
    ...
    try:
        ...
        return jsonify({"message": "company updated successfully", "results": company_schema.dump(company)}), 200
    except:
        db.session.rollback()
...
def get_company_by_id(req, company_id):
    ...
    return jsonify({"message": "company requested", "results": company_schema.dump(company)}), 200
...
def delete_company(company_id):
    ...
    try:
        delete_company_data = product_schema.dump(company)
