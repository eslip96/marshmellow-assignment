...
def get_product_by_id(req, product_id):
    ...
    return jsonify({"message": "product requested", "results": product_schema.dump(product)}), 200
...
def get_products_by_company_id(req, company_id):
    try:
        product = db.session.query(Products).filter(Products.company_id == company_id).all()
        if not product:
            return jsonify({"message": "no products found for the given company id."}), 404
        return jsonify({"message": "products retrieved successfully.", "results": products_schema.dump(product)}), 200
    except:
        return jsonify({"message": "failed to retrieve products"}), 400
